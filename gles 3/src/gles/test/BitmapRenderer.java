...
public class BitmapRenderer implements Renderer {
    ...
    public BitmapRenderer(Bitmap bitmap1, Bitmap bitmap2, float r, float g, float b, float a, String shader) {
        this(bitmap1, bitmap2, r, g, b, a, shader, LEFT);
    }

    public BitmapRenderer(Bitmap bitmap1, Bitmap bitmap2, float r, float g, float b, float a, String shader, int rotation) {
        ...
        if (bitmap2 != null) {
            textureBuffer = ByteBuffer.allocateDirect(texcoord0.length * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();

            if (bitmap1.getWidth() > bitmap1.getHeight()) {
                if (bitmap2.getWidth() > bitmap2.getHeight()) {
                    textureBuffer.put(texcoord0).position(0);
                } else {
                    if (rotation == RIGHT) {
                        textureBuffer.put(texcoord90).position(0);
                    } else {
                        textureBuffer.put(texcoord270).position(0);
                    }
                }
            } else {
                if (bitmap2.getWidth() < bitmap2.getHeight()) {
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (bitmap2 != null) {
            GLES20.glVertexAttribPointer(iTexCoords2, 2, GLES20.GL_FLOAT, false, 0, textureBuffer);
